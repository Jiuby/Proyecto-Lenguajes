...
-- Flujo:
--   - Pide el ID
--   - Si ya esta adentro => avisa y no hace nada
--   - Si no esta adentro => busca sus datos en University.txt
--                        => si no esta en el archivo, los pide manualmente
...
registrarEntrada registros = do
    ...
    -- 'find' recorre la lista buscando el primero que cumpla la condicion.
    -- Aqui buscamos un registro activo (mismo ID y todavia adentro).
    case find (\r -> regId r == sid && estaAdentro r) registros of

        Just _ -> do
            -- Ya existe un registro activo => no hacemos nada
            putStrLn "[!] El estudiante ya esta registrado adentro."
            return registros

        Nothing -> do
            -- No esta adentro, procedemos a registrar su entrada
            ...
            (nombre, carrera) <- case find (\e -> estId e == sid) estudiantes of
                ...
                Nothing -> do
                    -- No esta en el archivo, pedimos los datos a mano
            ...
            -- porque el estudiante acaba de entrar y no ha salido aun.
            ...
            putStrLn $ "[OK] Entrada registrada: " ++ nombre ++ " a las " ++ hora
            return actualizado
...
-- ----------------------------------------------------------------
-- Muestra la informacion de un estudiante si esta actualmente adentro.
...
buscarEstudiante registros = do
    ...
    case find (\r -> regId r == sid && estaAdentro r) registros of
        Nothing ->
            putStrLn "[X] Estudiante no encontrado o ya registro salida."
        Just r -> do
...
--
-- Usa horaAMinutos para convertir a numeros y poder restar.
...
calcularTiempo = do
    ...
    if diff < 0
        then putStrLn "[X] La hora de salida es anterior a la de entrada."
        else putStrLn $ "[OK] Tiempo en la universidad: "
                     ++ minutosAHora diff
                     ++ "  (" ++ show diff ++ " minutos)"
...
listarEstudiantes = do
    estudiantes <- cargarEstudiantes "University.txt"
    if null estudiantes
        then putStrLn "[X] No hay estudiantes en University.txt."
        else do
...
-- ----------------------------------------------------------------
-- Registra la salida de un estudiante y calcula cuanto tiempo estuvo.
--
-- Flujo:
--   - Pide el ID
--   - Si no esta adentro => avisa y no hace nada
--   - Si esta adentro    => pide la hora de salida
--                        => actualiza su registro con esa hora
--                        => guarda en records.txt
--                        => muestra cuanto tiempo estuvo
--
-- Recibe: la lista actual de registros
...
registrarSalida registros = do
    ...
    case find (\r -> regId r == sid && estaAdentro r) registros of
        ...
        Just r -> do
            ...
            -- Solo modifica el que tenga el mismo ID y este adentro.
            -- Los demas los deja exactamente igual.
            ...
            putStrLn $ "[OK] Salida registrada a las " ++ hora
...
-- ----------------------------------------------------------------
-- marcarSalida (funcion auxiliar de registrarSalida)
-- ----------------------------------------------------------------
